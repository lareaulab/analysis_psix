...
rd <- read.table('tables/pc2_rd.tab.gz', sep = '\t', header = TRUE, row.names = 1)
tpm_norm_counts <- read.table('tables/scone_norm_tpm.tab.gz', sep = '\t', header = TRUE, row.names = 1)[, rownames(rd)]
sc_meta <- read.table('../SraRunTable.txt.gz', sep = ',', header = TRUE, row.names = 1)[rownames(rd),]
...
write.table(pseudo_curve, 'tables/pseudotime.tab', quote = FALSE, sep='\t')
