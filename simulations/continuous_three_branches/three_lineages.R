...
true_counts_res <- SimulateTrueCounts(ncells_total=ncells_total, ngenes=ngenes, nevf=20, evf_type="continuous", n_de_evf=12, vary="s", Sigma=0.4, phyla=phylo_3tips, randseed=1)
...
if(l==1){
  ...
  h_vector <= c()
  if (hx < 0.7) {h_vector = c(h_vector, runif(1, min=logit(expit(hx)+0.2), max=3))}
  if (hx > 0.3) {h_vector = c(h_vector, runif(1, min=-3, max=logit(expit(hx)-0.2)))}
...
l1_diff = rbinom(ngenes, 1, 0.2)
l2_diff = rbinom(ngenes, 1, 0.2)
l3_diff = rbinom(ngenes, 1, 0.2)
