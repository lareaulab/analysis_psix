...
def get_distance_matrix(pca, k=100):
    ...
    nbrs = NearestNeighbors(n_neighbors=k+1).fit(pca)
...
tiklova_pca = pd.read_csv('~/analysis_psix/scvi_runs/tiklova_scvi5_rd.tab', sep='\t', index_col=0)
...
tiklova_mock_dict = get_mock_dict(tiklova_PSI.loc[good_exons], tiklova_norm_PSI.loc[good_exons], W_tiklova, mock=2000)
...
pgamma_tiklova.to_csv('tiklova_GearyC_scvi5.tab', sep='\t', header=True, index=True)
